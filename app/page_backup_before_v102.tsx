...
const YOUTUBE_MUSIC_URL = process.env.NEXT_PUBLIC_RUNE_RUSH_YOUTUBE_MUSIC ?? "";
...
const DEFAULT_AUDIO_SETTINGS: AudioSettings = { gameSounds: true, music: false, gameVolume: 0.78, musicVolume: 0.22 };
...
function cleanAudioSettings(value: unknown): AudioSettings {
  ...
  return {
    ...
    gameVolume: cleanVolume(raw.gameVolume, DEFAULT_AUDIO_SETTINGS.gameVolume),
    musicVolume: cleanVolume(raw.musicVolume, DEFAULT_AUDIO_SETTINGS.musicVolume),
  };
}
...
export default function RuneRushPixiFullPage() {
  ...
  const boardKey = useMemo(() => `pixi-full-v99-${levelIndex}-${resetKey}`, [levelIndex, resetKey]);
  ...
  const gameVolumePct = Math.round(audioSettings.gameVolume * 100);
  const musicVolumePct = Math.round(audioSettings.musicVolume * 100);
  const youtubeMusicId = getYouTubeVideoId(YOUTUBE_MUSIC_URL);
  const hasYouTubeMusic = Boolean(youtubeMusicId);
  ...
  const setAllAudio = (enabled: boolean) => {
    setAudioSettings((prev) => cleanAudioSettings({ ...prev, gameSounds: enabled, music: enabled }));
  };

  const getMusicAudio = () => {
    if (typeof window === "undefined") return null;
    if (hasYouTubeMusic) return null;
    ...
    audio.addEventListener("error", () => {
      if (musicIndexRef.current >= MUSIC_CANDIDATES.length - 1) return;
  ...
  useEffect(() => {
    ...
    if (musicAudioRef.current) musicAudioRef.current.volume = audioSettings.musicVolume;
    if (youtubePlayerRef.current?.setVolume) {
      youtubePlayerRef.current.setVolume(Math.round(audioSettings.musicVolume * 100));
    }
  }, [audioSettings]);

  useEffect(() => {
    if (!hasYouTubeMusic || typeof window === "undefined") return;
    ...
    const startPlayer = () => {
      ...
      if (youtubePlayerRef.current) {
        applyYouTubeVolume();
        if (musicOn) youtubePlayerRef.current.playVideo?.();
        else youtubePlayerRef.current.pauseVideo?.();
        return;
      }
      if (!musicOn || !w.YT?.Player) return;

      youtubePlayerRef.current = new w.YT.Player(youtubeHostRef.current, {
        height: "1",
        width: "1",
        ...
        playerVars: {
          ...
          loop: 1,
          modestbranding: 1,
          playlist: youtubeMusicId,
          playsinline: 1,
          rel: 0,
        },
        events: {
          onReady: (event: any) => {
            event.target.setVolume(Math.round(audioSettings.musicVolume * 100));
            if (musicOn) event.target.playVideo();
          },
          onStateChange: (event: any) => {
            if (event.data === 0 && musicWantedRef.current) event.target.playVideo();
          },
        },
      });
    };
  ...
  }, [hasYouTubeMusic, youtubeMusicId, musicOn, audioSettings.musicVolume]);
  ...
  return (
    <main className="pageShell">
      ...
      <section className="goalCard">
        <div className={`goalIcon ${hud.objectiveKind}`}>
          {hud.objectiveKind === "ingredient" ? (
            <span className="goalIconIngredients" aria-hidden="true">
              <img src="/ingredients/key.png?v=hub101" alt="" draggable={false} />
              <img src="/ingredients/coin.png?v=hub101" alt="" draggable={false} />
            </span>
          ) : (
            objectiveIcon(hud.objectiveKind)
          )}
        </div>
        <div className="goalMain">
              ...
              <div className="goalTitle">
                ...
                {hud.objectiveKind === "ingredient" && hud.ingredient && (
                  <span className="goalInlineIngredient" aria-label="Sacred Key and Golden Coin remaining">
                    <img className="ingredientMini keyMini" src="/ingredients/key.png?v=hub101" alt="" draggable={false} />
                    <img className="ingredientMini coinMini" src="/ingredients/coin.png?v=hub101" alt="" draggable={false} />
                    <b>{hud.ingredient.total - hud.ingredient.dropped}</b>
                  </span>
                )}
              </div>
            </div>
          ...
          {hud.objectiveKind === "ingredient" && (
            <div className="ingredientHubShowcase" aria-label="Sacred Key and Golden Coin">
              <img src="/ingredients/key.png?v=hub102" alt="Sacred Key" draggable={false} />
              <img src="/ingredients/coin.png?v=hub102" alt="Golden Coin" draggable={false} />
            </div>
          )}
      ...
      <section className="boardFrame">
        {canPlay ? (
          <RuneRushPixiBoard
            ...
            soundVolume={audioSettings.gameVolume}
      ...
      <footer className="bottomBar">
        <button
          className="arrowBtn controlArrow"
          ...
          {soundMenuOpen && (
            <div id="audioSettingsMenu" className="soundMenu" role="menu" aria-label="Audio settings">
              ...
              <label className="volumeControl">
                <span>Game volume <b>{gameVolumePct}%</b></span>
                <input
                  type="range"
                  min="0"
                  max="100"
              ...
              <label className="volumeControl">
                <span>Music volume <b>{musicVolumePct}%</b></span>
                <input
                  type="range"
                  min="0"
                  max="100"
        ...
        <button
          className={levelIndex + 1 > highestUnlocked ? "arrowBtn controlArrow locked" : "arrowBtn controlArrow"}
          onClick={() => startLevel(levelIndex + 1)}
      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; }
        ...
        .goalIconIngredients img {
          position: absolute;
          width: 30px;
          height: 30px;
          object-fit: contain;
          filter: brightness(1.18) contrast(1.12) saturate(1.1) drop-shadow(0 2px 4px rgba(0,0,0,0.45));
        }

        .goalIconIngredients img:first-child {
          transform: translate(-6px, -1px) rotate(-11deg) scale(1.12);
        }

        .goalIconIngredients img:last-child {
        ...
        .ingredientMini {
          ...
          height: 27px;
          object-fit: contain;
          filter: brightness(1.18) contrast(1.13) saturate(1.1) drop-shadow(0 2px 4px rgba(0,0,0,0.48));
        }
        .keyMini { transform: rotate(-9deg) scale(1.18); }
        .coinMini { transform: scale(1.02); }
        ...
        .ingredientHubShowcase img {
          ...
          height: 38px;
          object-fit: contain;
          filter: brightness(1.18) contrast(1.13) saturate(1.1) drop-shadow(0 3px 5px rgba(0,0,0,0.5));
        }
        ...
        @media (max-width: 460px) {
          ...
          .bottomBar { grid-template-columns: 42px minmax(0, 1fr) minmax(0, 1fr) 42px; gap: 8px; }
          .bottomBar button { height: 44px; font-size: 13px; }
          .controlArrow { font-size: 28px; }
        }
