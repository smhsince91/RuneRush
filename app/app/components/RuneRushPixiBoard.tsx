...
const BUILD_TAG = "pixifull-v62-sound-lives-fix";
...
function getLevel(levelIndex: number): Level {
  ...
  // Smooth difficulty curve: levels slowly get harder, but breather levels appear so
  // progress never turns into an impossible wall.
  const breather = num > 2 && (num % 7 === 2 || num % 11 === 5 || num % 13 === 8);
  const difficulty = Math.min(18, Math.floor(levelIndex * 0.55));
  const baseMoves = clamp((breather ? 20 : 18) - Math.floor(levelIndex * 0.18), breather ? 15 : 11, breather ? 21 : 18);
  const targetScore = Math.round((breather ? 780 : 950) + levelIndex * (breather ? 105 : 150) + Math.floor(levelIndex / 6) * 90);

  if (num % 3 === 0) {
    return {
      idx: levelIndex,
      moves: clamp((breather ? 22 : 20) - Math.floor(levelIndex * 0.14), breather ? 16 : 12, breather ? 22 : 20),
      objectiveKind: "fog",
      targetScore,
      collectTarget: {},
      fogCount: clamp((breather ? 4 : 6) + Math.floor(difficulty * 0.72), breather ? 4 : 6, breather ? 15 : 23),
    };
  }
  ...
  if (num % 4 === 0 || num % 5 === 0) {
    ...
    const base = clamp((breather ? 7 : 10) + Math.floor(difficulty * 0.78), breather ? 7 : 10, breather ? 19 : 26);
    const collectTarget: Partial<Record<Rune, number>> = { [first]: base };
    if (!breather && num >= 10) collectTarget[second] = clamp(base - 4, 7, 22);
    return {
      idx: levelIndex,
      moves: baseMoves + (breather ? 2 : 1),
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const drawBoardBack = () => {
        ...
        slab.fill({ color: 0x121517, alpha: 1 });
        board.addChild(slab);
        ...
        if (stoneTex) {
          ...
          boardStone.alpha = 0.72;
          boardStone.tint = 0xb0b8b2;
        ...
        obsidianDepth.fill({ color: 0x14181a, alpha: 0.38 });
        ...
        bevelDark.stroke({ color: 0x000000, alpha: 0.18, width: Math.max(1, Math.round(tileSize * 0.018)) });
      ...
      const createTileView = (t: Tile, start?: { r: number; c: number }, spawned = false) => {
        ...
        const cellShadow = new Graphics();
        cellShadow.roundRect(-socketSize / 2, -socketSize / 2 + tileSize * 0.06, socketSize, socketSize, radius);
        cellShadow.fill({ color: 0x000000, alpha: 0.24 });
        wrap.addChild(cellShadow);

        const socket = new Graphics();
        socket.roundRect(-socketSize / 2, -socketSize / 2, socketSize, socketSize, radius);
        socket.fill({ color: 0x0b0e10, alpha: 0.58 });
        wrap.addChild(socket);

        const stoneMask = new Graphics();
        stoneMask.roundRect(-socketSize / 2, -socketSize / 2, socketSize, socketSize, radius);
        ...
        const polishedGem = new Graphics();
        polishedGem.roundRect(-socketSize / 2 + 1, -socketSize / 2 + 1, socketSize - 2, socketSize - 2, Math.max(10, radius - 1));
        polishedGem.fill({ color: 0x050709, alpha: 0.18 });
        wrap.addChild(polishedGem);

        const socketSheen = new Graphics();
        socketSheen.roundRect(-socketSize / 2 + 3, -socketSize / 2 + 3, socketSize - 6, socketSize * 0.32, Math.max(9, radius - 2));
        socketSheen.fill({ color: 0xffffff, alpha: 0.014 });
        wrap.addChild(socketSheen);
        ...
        if (tex) {
          ...
          const runeShadow = new Graphics();
          runeShadow.ellipse(0, crispRuneSize * 0.31, crispRuneSize * 0.30, crispRuneSize * 0.09);
          runeShadow.fill({ color: 0x000000, alpha: 0.30 });
          wrap.addChild(runeShadow);
      ...
      const playSmallPop = async (cellIds: number[]) => {
        if (cellIds.length >= 7) playSfx("runicClear");
      ...
      const clearCells = async (
        ...
        const protectedSpawnCell = spawn?.k;
        if (protectedSpawnCell != null) clearSet.delete(protectedSpawnCell);
        ...
        if (spawn) {
          ...
          if (t) {
            t.rune = spawn.rune;
            t.special = spawn.special;
            if (spawn.special !== "none") playSfx("spawn");
            playSpawnRing(loc.r, loc.c, spawn.special);
          }
        }
      ...
      const playSpawnRing = (r: number, c: number, special: Special) => {
        ...
        diamond.roundRect(-tileSize * 0.18, -tileSize * 0.18, tileSize * 0.36, tileSize * 0.36, tileSize * 0.08);
        diamond.fill({ color: 0xffffff, alpha: 0.18 });
        diamond.x = cx;
        diamond.y = cy;
        diamond.rotation = Math.PI / 4;
        diamond.blendMode = "add" as any;
        fxLayer.addChild(diamond);
        premiumGlow(diamond, color, 360, special === "lotus" ? 1.35 : 1.05);

        luxuryParticleBurst(["sparkle", "star", "spark"], cx, cy, special === "lotus" ? 10 : 7, tileSize * 0.56, tileSize * 0.18, color, 250);

        Promise.all([
          gsapTo(diamond.scale, { x: 2.15, y: 2.15 }, 260, "power4.out"),
          gsapTo(diamond, { alpha: 0, rotation: diamond.rotation + 0.18 }, 260, "power3.out"),
      ...
      const playMassiveGoldenClear = async (a: Tile, b?: Tile) => {
        playSfx("golden");
        setTimeout(() => playSfx("runicClear"), 220);
      ...
      const showRoyalLotusBloom = async (origin: Tile) => {
        ...
        setTimeout(() => lotusLuxuryGlowBurst(cx, cy, 0xfff1a8, 1.05, 1.1), 120);
      ...
      const playRoyalLotusBloom = async (a: Tile, b?: Tile) => {
        playSfx("lotus");
        setTimeout(() => playSfx("runicClear"), 420);
...
const playLotus = async (origin: Tile, color: Rune) => {
      ...
      app.ticker.add((ticker: any) => {
        ...
        for (const tv of tileViews.values()) {
          if (!tv.wrap || tv.wrap.destroyed) continue;
          tv.wrap.scale.set(1);
        }
