...
function CollectRuneIcon({ rune }: { rune: Rune }) {
  ...
  if (!src) {
    return <span className="chipRune fallbackOnly" aria-label={rune}>{RUNE_FALLBACK[rune]}</span>;
  }

  return (
    <span className="chipRune" aria-label={rune}>
      <span className="chipRuneFallback" aria-hidden="true">{RUNE_FALLBACK[rune]}</span>
      <img
        src={`${src}?v=collect82`}
        alt={rune}
        draggable={false}
