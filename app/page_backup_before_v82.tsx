...
function CollectRuneIcon({ rune }: { rune: Rune }) {
  ...
  if (failed) {
    return <span className="chipRune fallbackOnly">{RUNE_FALLBACK[rune]}</span>;
  }

  return (
    <span className="chipRune">
      <img
        src={`${RUNE_ICON_FILES[rune]}?v=collect24`}
        alt={rune}
        draggable={false}
...
export default function RuneRushPixiFullPage() {
  ...
  const boardKey = useMemo(() => `pixi-full-v73-${levelIndex}-${resetKey}`, [levelIndex, resetKey]);
      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; }
        ...
        .chipRune img {
          ...
          object-fit: contain;
          display: block;
          filter: drop-shadow(0 0 6px rgba(255, 245, 190, 0.28));
        }
