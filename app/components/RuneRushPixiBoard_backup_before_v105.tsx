...
type Ingredient = { id: number; r: number; c: number; dropped: boolean; kind: IngredientKind };
...
const BUILD_TAG = "pixifull-v103-audio-volume-device-pass";
    ...
    const run = async () => {
      ...
      const createTileView = (t: Tile, start?: { r: number; c: number }, spawned = false) => {
        ...
        if (occupiedByIngredient) {
          // Pond Relic cells use their own carved slot. No rune is drawn underneath,
          // so the relic never looks like it is sitting on top of a normal rune image.
          const relicSocket = new Graphics();
          relicSocket.roundRect(-socketSize * 0.28, -socketSize * 0.31, socketSize * 0.56, socketSize * 0.62, Math.max(8, radius - 8));
          relicSocket.fill({ color: 0x1d1710, alpha: 0.64 });
          relicSocket.stroke({ color: 0xffd978, alpha: 0.23, width: Math.max(1.4, tileSize * 0.022) });
          wrap.addChild(relicSocket);

          const relicShadow = new Graphics();
          relicShadow.ellipse(0, socketSize * 0.21, socketSize * 0.23, socketSize * 0.055);
          relicShadow.fill({ color: 0x000000, alpha: 0.18 });
          wrap.addChild(relicShadow);
        } else if (tex) {
      ...
      const renderIngredients = () => {
        ...
        for (const item of ingredients) {
          ...
          // Guaranteed-visible Pixi Graphics ingredients: actual Sacred Key and Golden Coin.
          // These do not use emoji or optional image files, so they show on phones, laptops, and Base App.
          const halo = new Graphics();
          halo.circle(0, 0, tileSize * 0.35);
          halo.fill({ color: 0xffd978, alpha: 0.15 });
          halo.blendMode = "add" as any;
          wrap.addChild(halo);

          if (item.kind === "coin") {
            const coinShadow = new Graphics();
            coinShadow.ellipse(0, tileSize * 0.20, tileSize * 0.22, tileSize * 0.052);
            coinShadow.fill({ color: 0x000000, alpha: 0.18 });
            wrap.addChild(coinShadow);

            const coin = new Graphics();
            coin.circle(0, 0, tileSize * 0.245);
            coin.fill({ color: 0xd99a2f, alpha: 1 });
            coin.stroke({ color: 0xfff0b7, alpha: 0.92, width: Math.max(1.6, tileSize * 0.026) });
            wrap.addChild(coin);

            const innerRing = new Graphics();
            innerRing.circle(0, 0, tileSize * 0.170);
            innerRing.stroke({ color: 0xffcb61, alpha: 0.66, width: Math.max(1.0, tileSize * 0.018) });
            wrap.addChild(innerRing);

            const stamp = new Graphics();
            stamp.ellipse(0, tileSize * 0.018, tileSize * 0.085, tileSize * 0.055);
            stamp.fill({ color: 0x6f4212, alpha: 0.26 });
            stamp.circle(-tileSize * 0.050, -tileSize * 0.012, tileSize * 0.018);
            stamp.fill({ color: 0x6f4212, alpha: 0.26 });
            stamp.circle(tileSize * 0.050, -tileSize * 0.012, tileSize * 0.018);
            stamp.fill({ color: 0x6f4212, alpha: 0.26 });
            wrap.addChild(stamp);
            ...
            shine.ellipse(-tileSize * 0.080, -tileSize * 0.090, tileSize * 0.050, tileSize * 0.085);
            shine.fill({ color: 0xffffff, alpha: 0.40 });
            shine.rotation = -0.52;
            wrap.addChild(shine);
          } else {
            const keyShadow = new Graphics();
            keyShadow.ellipse(tileSize * 0.010, tileSize * 0.205, tileSize * 0.23, tileSize * 0.052);
            keyShadow.fill({ color: 0x000000, alpha: 0.18 });
            wrap.addChild(keyShadow);

            const key = new Graphics();
            const shaftW = Math.max(3.5, tileSize * 0.060);
            key.circle(-tileSize * 0.115, -tileSize * 0.045, tileSize * 0.125);
            key.stroke({ color: 0xfff0b7, alpha: 0.94, width: shaftW });
            key.circle(-tileSize * 0.115, -tileSize * 0.045, tileSize * 0.050);
            key.stroke({ color: 0x774914, alpha: 0.38, width: Math.max(1.0, tileSize * 0.018) });
            key.roundRect(-tileSize * 0.010, -tileSize * 0.075, tileSize * 0.300, tileSize * 0.060, 999);
            key.fill({ color: 0xd99a2f, alpha: 1 });
            key.stroke({ color: 0xfff0b7, alpha: 0.88, width: Math.max(1.2, tileSize * 0.018) });
            key.roundRect(tileSize * 0.210, -tileSize * 0.020, tileSize * 0.060, tileSize * 0.115, Math.max(1, tileSize * 0.010));
            key.fill({ color: 0xd99a2f, alpha: 1 });
            key.roundRect(tileSize * 0.130, -tileSize * 0.016, tileSize * 0.052, tileSize * 0.090, Math.max(1, tileSize * 0.010));
            key.fill({ color: 0xd99a2f, alpha: 1 });
            key.rotation = -0.20;
            wrap.addChild(key);

            const shine = new Graphics();
            shine.roundRect(-tileSize * 0.045, -tileSize * 0.090, tileSize * 0.190, tileSize * 0.014, 999);
      ...
      const applyIngredientFall = (clearSet: Set<number>) => {
        ...
        for (const item of ingredients) {
          ...
          if (item.r >= N - 1) {
            ...
            score += 150;
            showIngredientDropFx(Math.min(N - 1, oldR), item.c, item.kind);
          }
        }
        if (changed) hud();
      };
      ...
      const playGoldenFlash = async (origin: Tile, dir: "h" | "v") => {
        ...
        const spawnMovingHead = (side: -1 | 1) => {
          ...
          const endX = isH ? cx + side * endpointD * tileSize : cx;
          const endY = isH ? cy : cy + side * endpointD * tileSize;
          Promise.all([
        ...
        const spawnSlashSegment = (tx: number, ty: number, side: number) => {
          ...
          const makeStroke = (width: number, tint: number, alpha: number, jitter = 0) => {
            const g = new Graphics();
            const length = tileSize * (side === 0 ? 0.72 : 1.03);
            if (isH) {
        ...
        await wait(maxD * impactDelay + 245);
      };
...
const playLotus = async (origin: Tile, color: Rune, swappedWith?: Tile) => {
      ...
      const runRuneRushFinale = async () => {
        ...
        addBoardBannerText("RUNIC CLEAR!");
        await wait(unusedMoves > 0 ? 470 : 260);
