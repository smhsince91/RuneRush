...
type Ingredient = { id: number; r: number; c: number; dropped: boolean };
...
const BUILD_TAG = "pixifull-v97-level-complete-ingredients-device-pass";
...
function getLevel(levelIndex: number): Level {
  ...
  // Goal: drop ancient ingredients to the bottom by clearing under them.
  if (num === 2 || num % 6 === 2) {
    ...
    return {
      ...
      collectTarget: {},
      fogCount: 0,
      ingredientTarget: clamp(ingredientBase[tier] + Math.floor(wave * 0.35), ingredientBase[tier], 4),
    };
  }
...
function initIngredients(count: number): Ingredient[] {
  ...
  for (let i = 0; i < safeCount; i++) {
    ingredients.push({ id: i + 1, r: startRows[i] ?? 0, c: columns[i % columns.length], dropped: false });
  }
  return ingredients;
}
    ...
    const run = async () => {
      ...
      const hud = () => {
        ...
        if (level.objectiveKind === "collect") {
          ...
          objectiveLabel = "Drop Ingredients";
      ...
      const renderIngredients = () => {
        ...
        for (const item of ingredients) {
          ...
          // Guaranteed-visible Pixi Graphics ingredient: no emoji/image dependency across devices.
          const aura = new Graphics();
          aura.circle(0, 0, tileSize * 0.31);
          aura.fill({ color: 0xffe29a, alpha: 0.16 });
          aura.blendMode = "add" as any;
          wrap.addChild(aura);

          const seed = new Graphics();
          seed.ellipse(0, tileSize * 0.01, tileSize * 0.18, tileSize * 0.25);
          seed.fill({ color: 0xffc96d, alpha: 0.94 });
          seed.stroke({ color: 0xfff0b8, alpha: 0.88, width: Math.max(1.5, tileSize * 0.028) });
          wrap.addChild(seed);

          const shine = new Graphics();
          shine.ellipse(-tileSize * 0.055, -tileSize * 0.07, tileSize * 0.055, tileSize * 0.10);
          shine.fill({ color: 0xffffff, alpha: 0.45 });
          shine.rotation = -0.34;
          wrap.addChild(shine);

          const arrow = new Graphics();
          arrow.moveTo(-tileSize * 0.095, tileSize * 0.33);
          arrow.lineTo(0, tileSize * 0.44);
          arrow.lineTo(tileSize * 0.095, tileSize * 0.33);
          arrow.stroke({ color: 0xfff2c4, alpha: 0.72, width: Math.max(2, tileSize * 0.035) });
          wrap.addChild(arrow);
        }
      };
      ...
      const showIngredientDropFx = (r: number, c: number) => {
        const p = xy(Math.min(N - 1, Math.max(0, r)), c);
        const cx = p.x + tileSize / 2;
        const cy = p.y + tileSize / 2;
        playSfx("special");
        addScorePopup(cx, cy, "+ingredient", "special");
      ...
      const applyIngredientFall = (clearSet: Set<number>) => {
        ...
        for (const item of ingredients) {
          ...
          if (item.r >= N - 1) {
            ...
            score += 150;
            showIngredientDropFx(Math.min(N - 1, oldR), item.c);
          }
        }
        if (changed) hud();
      };
      ...
      const resolveCascades = async (firstSpawn: ReturnType<typeof chooseSpawn> | null) => {
        ...
        if (cascadeCount >= maxCascadeLoops) {
          repairGrid();
          if (computeMatches(tiles, fog).clear.size > 0) {
      ...
      const playGoldenFlash = async (origin: Tile, dir: "h" | "v") => {
        ...
        const hot = 0xffffff;
        const isH = dir === "h";
        const startX = isH ? pad : cx;
        const startY = isH ? cy : pad;
        const endX = isH ? pad + len : cx;
        const endY = isH ? cy : pad + len;

        // v95 visible-device Golden swipe:
        // The core effect is pure Pixi Graphics with normal alpha, not only filters/textures/blend modes.
        // That makes the lightning visible in Chrome, mobile Safari, Android WebView, and Base App browsers.
        await pulseTileView(origin, 1.12, 38, 48);
        microShake(0.55, 62);
        ...
        const makeSegment = (half: number, tint: number, alpha: number, offset = 0) => {
          const g = new Graphics();
          if (isH) g.roundRect(0, -half + offset, len, half * 2, 999);
          else g.roundRect(-half + offset, 0, half * 2, len, 999);
          g.fill({ color: tint, alpha });
          beam.addChild(g);
          return g;
        };

        const shadow = makeSegment(tileSize * 0.125, 0x2d1600, 0.28);
        const wideGlow = makeSegment(tileSize * 0.105, color, 0.30);
        const aura = makeSegment(tileSize * 0.064, amber, 0.44);
        const core = makeSegment(Math.max(2.0, tileSize * 0.030), color, 0.92);
        const whiteHot = makeSegment(Math.max(1.2, tileSize * 0.014), hot, 0.98);
        // A second offset filament makes the line read as lightning even when the frame rate is low.
        const offsetBolt = makeSegment(Math.max(1.0, tileSize * 0.010), hot, 0.58, tileSize * 0.055);
        const offsetBolt2 = makeSegment(Math.max(0.8, tileSize * 0.008), 0xfff0b0, 0.46, -tileSize * 0.052);

        beam.scale.x = isH ? 0.035 : 1;
        beam.scale.y = isH ? 1 : 0.035;
        try {
          premiumGlow(beam, color, 260, 1.45);
          premiumBloom(beam, 245, 0.66);
          premiumMotionBlur(beam, dir, 170);
        } catch {}

        const jagged = new Container();
        jagged.x = startX;
        jagged.y = startY;
        jagged.alpha = 0;
        fxLayer.addChild(jagged);
        const makeJagged = (boltIndex: number) => {
          const bolt = new Graphics();
          const steps = 9;
          const side = boltIndex % 2 === 0 ? 1 : -1;
          const jitter = tileSize * (boltIndex === 0 ? 0.13 : 0.21);
          if (isH) {
            bolt.moveTo(0, side * tileSize * 0.012);
            for (let i = 1; i <= steps; i++) {
              bolt.lineTo((len / steps) * i, side * tileSize * 0.012 + (Math.random() - 0.5) * jitter);
            }
          } else {
            bolt.moveTo(side * tileSize * 0.012, 0);
            for (let i = 1; i <= steps; i++) {
              bolt.lineTo(side * tileSize * 0.012 + (Math.random() - 0.5) * jitter, (len / steps) * i);
            }
          }
          bolt.stroke({
            color: boltIndex === 0 ? hot : boltIndex === 1 ? 0xffedb0 : color,
            alpha: boltIndex === 0 ? 0.88 : 0.56,
            width: boltIndex === 0 ? Math.max(1.5, tileSize * 0.026) : Math.max(1.0, tileSize * 0.016),
          });
          jagged.addChild(bolt);
          return bolt;
        };
        const bolts = [makeJagged(0), makeJagged(1), makeJagged(2)];

        const head = new Graphics();
        head.circle(0, 0, tileSize * 0.115);
        head.fill({ color: hot, alpha: 0.92 });
        head.circle(0, 0, tileSize * 0.225);
        head.fill({ color, alpha: 0.24 });
        head.x = cx;
        head.y = cy;
        head.alpha = 1;
        fxLayer.addChild(head);
        try { premiumGlow(head, color, 220, 1.25); } catch {}

        const originRing = new Graphics();
        originRing.circle(0, 0, tileSize * 0.29);
        originRing.stroke({ color: hot, alpha: 0.72, width: Math.max(1.7, tileSize * 0.030) });
        originRing.x = cx;
        originRing.y = cy;
        originRing.scale.set(0.42);
        fxLayer.addChild(originRing);

        candySparkBurst(cx, cy, color, 0.74);
        luxuryParticleBurst(["spark", "sparkle", "star"], cx, cy, 5, tileSize * 0.26, tileSize * 0.14, color, 170);
        gsapTo(originRing.scale, { x: 1.42, y: 1.42 }, 150, "power4.out");
        gsapTo(originRing, { alpha: 0 }, 160, "power2.out").then(() => originRing.destroy());

        const touched = isH
          ? Array.from({ length: N }, (_, c) => ({ r: origin.r, c }))
          : Array.from({ length: N }, (_, r) => ({ r, c: origin.c }));

        const addImpact = (cell: { r: number; c: number }, i: number) => {
          const q = xy(cell.r, cell.c);
          const tx = q.x + tileSize / 2;
          const ty = q.y + tileSize / 2;
          const ring = new Graphics();
          ring.circle(0, 0, tileSize * 0.19);
          ring.stroke({ color: i % 2 ? hot : color, alpha: 0.66, width: Math.max(1.2, tileSize * 0.022) });
          ring.x = tx;
          ring.y = ty;
          ring.scale.set(0.50);
          fxLayer.addChild(ring);
          Promise.all([
            gsapTo(ring.scale, { x: 1.25, y: 1.25 }, 125, "power4.out"),
            gsapTo(ring, { alpha: 0 }, 135, "power2.out"),
          ]).then(() => ring.destroy());

          const hit = new Graphics();
          hit.roundRect(-tileSize * 0.09, -tileSize * 0.09, tileSize * 0.18, tileSize * 0.18, tileSize * 0.045);
          hit.fill({ color: i % 2 ? hot : color, alpha: 0.34 });
          hit.x = tx;
          hit.y = ty;
          hit.rotation = Math.PI / 4;
          fxLayer.addChild(hit);
          Promise.all([
            gsapTo(hit.scale, { x: 1.72, y: 1.72 }, 110, "power4.out"),
            gsapTo(hit, { alpha: 0, rotation: hit.rotation + 0.25 }, 126, "power2.out"),
          ]).then(() => hit.destroy());

          luxuryParticleBurst(["spark", "sparkle", "star"], tx, ty, 3, tileSize * 0.20, tileSize * 0.11, color, 135);
          const tile = tiles.find((tt) => tt.r === cell.r && tt.c === cell.c);
          const tv = tile ? tileViews.get(tile.id) : null;
          if (tv) {
            gsapTo(tv.wrap.scale, { x: 1.105, y: 1.105 }, 36, "back.out(1.35)")
              .then(() => gsapTo(tv.wrap.scale, { x: 1, y: 1 }, 54, "power3.out"));
          }
        };

        // Make it quick, but not so fast that low-FPS devices miss the whole move.
        await Promise.all([
          gsapTo(beam.scale, { x: 1, y: 1 }, 108, "power4.out"),
          gsapTo(jagged, { alpha: 1 }, 44, "power2.out"),
          gsapTo(head, { x: endX, y: endY }, 126, "power4.out"),
        ]);

        touched.forEach((cell, i) => queueTimer(() => addImpact(cell, i), i * 14));
        await wait(118);

        await Promise.all([
          gsapTo(beam, { alpha: 0 }, 92, "power3.out"),
          gsapTo(jagged, { alpha: 0 }, 86, "power2.out"),
          gsapTo(head, { alpha: 0 }, 78, "power2.out"),
          gsapTo(head.scale, { x: 1.85, y: 1.85 }, 78, "power3.out"),
        ]);
