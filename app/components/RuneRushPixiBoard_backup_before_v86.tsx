...
const BUILD_TAG = "pixifull-v82-collect-icons-sound-performance";
...
export default function RuneRushPixiBoard({ levelIndex, onHud, soundOn = true, onLevelComplete, onLevelFailed }: Props) {
  ...
  const primeOriginalSfxBank = () => {
    ...
    // spawn.mp3, magic.mp3, zap.mp3, and clickclick.mp3 play more reliably and on time.
  ...
  useEffect(() => {
    ...
    for (const key of Object.keys(ORIGINAL_SFX) as SfxKey[]) {
      ...
      for (const src of ORIGINAL_SFX[key].slice(0, 5)) {
        ...
        audio.preload = "auto";
        audio.volume = key === "bad" ? 0.34 : key === "lotus" ? 0.55 : key === "spawn" ? 0.50 : key === "golden" ? 0.50 : key === "convert" ? 0.52 : 0.42;
  ...
  const playOriginalSfx = (key: SfxKey) => {
    ...
    // lotus -> /sfx/magic.mp3 first, spawn -> /sfx/spawn.mp3 first,
    // convert -> /sfx/clickclick.mp3 first. Clone ready sounds so rapid cascades
    // can overlap without one sound cutting off another.
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const clearCells = async (
        ...
        // v82: special-spawn audio is always spawn.mp3 and is fired after the tile lands/appears.
        // clickclick.mp3 is reserved for Rune Rush / Runic Clear conversion moments only.
