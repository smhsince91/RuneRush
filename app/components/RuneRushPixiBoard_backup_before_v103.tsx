...
const BUILD_TAG = "pixifull-v101-floating-relic-polish";
...
const INGREDIENT_FILES: Record<IngredientKind, string> = {
  key: "/ingredients/key.png",
  coin: "/ingredients/coin.png",
};
...
function getLevel(levelIndex: number): Level {
  ...
  // Ingredient levels appear early and then repeat, so players actually see the new objective.
  ...
  if (num === 2 || num % 6 === 2) {
    ...
    return {
      ...
      collectTarget: {},
      fogCount: 0,
      ingredientTarget: clamp(ingredientBase[tier] + Math.floor(wave * 0.35), 2, 4),
    };
  }
...
export default function RuneRushPixiBoard({ levelIndex, onHud, soundOn = true, soundVolume = 0.78, onLevelComplete, onLevelFailed }: Props) {
  const hostRef = useRef<HTMLDivElement | null>(null);
  const onHudRef = useRef(onHud);
  const onLevelCompleteRef = useRef(onLevelComplete);
  const onLevelFailedRef = useRef(onLevelFailed);
  const soundOnRef = useRef(soundOn);
  const soundVolumeRef = useRef(clamp(soundVolume, 0, 1));
  ...
  useEffect(() => {
    soundVolumeRef.current = clamp(soundVolume, 0, 1);
  }, [soundVolume]);
  ...
  const sfxVolume = () => clamp(soundVolumeRef.current, 0, 1);
  ...
  const playOriginalSfx = (key: SfxKey) => {
    ...
    for (const audio of candidates) {
      try {
        ...
        player.volume = clamp(audio.volume * sfxVolume(), 0, 1);
        player.muted = false;
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      let message = level.objectiveKind === "ingredient" ? "Guide the key and golden coin down" : "Swipe runes to match 3+";
      ...
      if (level.objectiveKind === "ingredient") {
        for (const item of ingredients) fog[idx(item.r, item.c)] = null;
      }
      let tiles = makeInitialTiles(fog);

      const activeIngredientCells = () => {
        const cells = new Set<number>();
        if (level.objectiveKind !== "ingredient") return cells;
      ...
      const hasActiveIngredientAt = (r: number, c: number) => activeIngredientCells().has(idx(r, c));
      ...
      const rendererResolution = isMobileView ? Math.min(dpr || 1, 2.15) : Math.min(dpr || 1, 2.5);
      const fxQuality = Math.max(0.66, Math.min(1.0, (tileSize / 52) * (deviceMemory <= 3 ? 0.72 : 1) * (isMobileView ? 0.82 : 1) * (dpr > 2.4 ? 0.86 : 1)));
      const allowPremiumFilters = fxQuality >= 0.78 && (!isMobileView || deviceMemory > 3);
      ...
      const ingredientTextures: Partial<Record<IngredientKind, Texture | null>> = {};
      for (const kind of ["key", "coin"] as IngredientKind[]) {
        const tex = await safeLoadTexture([`${INGREDIENT_FILES[kind]}?v=relic101`, INGREDIENT_FILES[kind]]);
        sharpenTexture(tex);
        ingredientTextures[kind] = tex;
      }
      ...
      const hud = () => {
        ...
        } else if (level.objectiveKind === "ingredient") {
          objectiveLabel = "Guide Sacred Key & Coin";
          const total = Math.max(1, level.ingredientTarget);
          objectiveText = `${ingredientDropped}/${total}`;
          progress = clamp(ingredientDropped / total, 0, 1);
          ingredient = { dropped: ingredientDropped, total };
        }
      ...
      const cascadePulsePause = async (wave: number, clearSize: number) => {
        const color = wave % 2 ? 0xffd7f4 : 0xffdc78;
        const cx = boardSize / 2;
        const cy = boardSize / 2;
        const pulse = new Graphics();
        pulse.roundRect(pad * 0.86, pad * 0.86, boardSize - pad * 1.72, boardSize - pad * 1.72, Math.max(16, tileSize * 0.34));
        pulse.stroke({ color, alpha: Math.min(0.24, 0.08 + clearSize * 0.006), width: Math.max(1.4, tileSize * 0.018) });
        pulse.x = 0;
        pulse.y = 0;
        ...
        center.circle(cx, cy, tileSize * 0.22);
        center.fill({ color, alpha: Math.min(0.12, 0.045 + clearSize * 0.0025) });
        center.blendMode = "add" as any;
        fxLayer.addChild(center);

        Promise.all([
          gsapTo(pulse.scale, { x: 1.015, y: 1.015 }, 84, "power2.out")
            .then(() => gsapTo(pulse.scale, { x: 1, y: 1 }, 90, "power2.out")),
          gsapTo(pulse, { alpha: 0 }, 190, "power2.out"),
          gsapTo(center.scale, { x: 4.4, y: 4.4 }, 170, "power3.out"),
          gsapTo(center, { alpha: 0 }, 180, "power2.out"),
        ]).then(() => {
          pulse.destroy();
          center.destroy();
        });

        await wait(wave > 1 ? 82 : 68);
      };
      ...
      const targetFromDelta = (tileId: number, dx: number, dy: number) => {
        ...
        tr = clamp(tr, 0, N - 1);
        tc = clamp(tc, 0, N - 1);
        if (fog[idx(tr, tc)] || hasActiveIngredientAt(tr, tc)) return null;
      ...
      const createTileView = (t: Tile, start?: { r: number; c: number }, spawned = false) => {
        ...
        if (occupiedByIngredient) return;
        ...
        if (tex) {
          ...
          runeClarityBack.circle(0, 0, tileSize * (t.special === "none" ? 0.24 : 0.29));
          runeClarityBack.fill({ color: 0xffffff, alpha: 0.035 });
          runeClarityBack.scale.set(1, 0.92);
          wrap.addChild(runeClarityBack);

          runeSprite = new Sprite(tex);
          runeSprite.anchor.set(0.5);
          // v98 Toby sizing pass:
          // Blue Toby and Golden Toby use the same canvas size so they render consistently across devices.
          // Golden was reading too small in v97, so it has been brought up to the same visual scale as Blue.
          const tobyRuneScale = 0.82;
          const runeScale =
            t.rune === "blue" ? tobyRuneScale :
            t.rune === "golden" ? tobyRuneScale :
            t.rune === "lotus" ? 0.78 :
            t.special === "none" ? 0.72 :
            0.76;
      ...
      const renderIngredients = () => {
        ...
        for (const child of oldIngredients) child.destroy({ children: true });
        if (level.objectiveKind !== "ingredient") return;
        for (const item of ingredients) {
          if (item.dropped) continue;
          const { x, y } = xy(item.r, item.c);
          const cx = x + tileSize / 2;
          const cy = y + tileSize / 2;
          const from = item.renderFrom;
          const start = from ? xy(from.r, from.c) : { x, y };
          const wrap = new Container();
          wrap.x = start.x + tileSize / 2;
          wrap.y = start.y + tileSize / 2;
          wrap.eventMode = "none";
          ingredientLayer.addChild(wrap);

          const float = new Container();
          (float as any).__ingredientFloat = {
            seed: item.id * 1.73 + (item.kind === "coin" ? 0.75 : 0),
            amp: tileSize * (item.kind === "coin" ? 0.046 : 0.056),
          };
          wrap.addChild(float);

          const tex = ingredientTextures[item.kind];
          if (tex) {
            const sprite = new Sprite(tex);
            sprite.anchor.set(0.5);
            const spriteSize = Math.round(tileSize * (item.kind === "coin" ? 1.12 : 1.26));
            sprite.width = spriteSize;
            sprite.height = spriteSize;
            sprite.y = item.kind === "coin" ? -tileSize * 0.015 : -tileSize * 0.060;
            sprite.rotation = item.kind === "key" ? -0.04 : 0;
            sprite.alpha = 1;
            sprite.tint = 0xffffff;
            (sprite as any).roundPixels = false;
            float.addChild(sprite);
          } else {
            const fallback = new Graphics();
            if (item.kind === "coin") {
              fallback.circle(0, -tileSize * 0.02, tileSize * 0.30);
              fallback.fill({ color: 0xb9812b, alpha: 1 });
              fallback.stroke({ color: 0xffe7a4, alpha: 0.82, width: Math.max(1.8, tileSize * 0.030) });
              fallback.circle(0, -tileSize * 0.02, tileSize * 0.20);
              fallback.stroke({ color: 0x5d3611, alpha: 0.42, width: Math.max(1.2, tileSize * 0.020) });
            } else {
              const shaftW = Math.max(3.5, tileSize * 0.060);
              fallback.circle(-tileSize * 0.115, -tileSize * 0.045, tileSize * 0.125);
              fallback.stroke({ color: 0xfff0b7, alpha: 0.94, width: shaftW });
              fallback.roundRect(-tileSize * 0.010, -tileSize * 0.075, tileSize * 0.300, tileSize * 0.060, 999);
              fallback.fill({ color: 0xd99a2f, alpha: 1 });
              fallback.roundRect(tileSize * 0.210, -tileSize * 0.020, tileSize * 0.060, tileSize * 0.115, Math.max(1, tileSize * 0.010));
              fallback.fill({ color: 0xd99a2f, alpha: 1 });
              fallback.rotation = -0.20;
            }
            float.addChild(fallback);
          }

          const glint = new Graphics();
          glint.roundRect(-tileSize * 0.010, -tileSize * 0.090, tileSize * 0.020, tileSize * 0.18, 999);
          glint.fill({ color: 0xffffff, alpha: 0.58 });
          glint.roundRect(-tileSize * 0.090, -tileSize * 0.010, tileSize * 0.18, tileSize * 0.020, 999);
          glint.fill({ color: 0xffffff, alpha: 0.38 });
          glint.x = tileSize * (item.kind === "coin" ? 0.20 : -0.24);
          glint.y = -tileSize * (item.kind === "coin" ? 0.24 : 0.28);
          glint.rotation = item.kind === "coin" ? 0.15 : -0.38;
          glint.blendMode = "add" as any;
          (glint as any).__ingredientGlint = { seed: item.id * 2.71 };
          float.addChild(glint);

          if (from) {
            const dist = Math.abs(from.r - item.r) + Math.abs(from.c - item.c);
            const dur = Math.min(360, 150 + dist * 66);
            void gsapTo(wrap, { x: cx, y: cy }, dur, "back.out(0.95)");
            item.renderFrom = undefined;
          } else {
            wrap.x = cx;
            wrap.y = cy;
          }
      ...
      const repairGrid = () => {
        const blocked = activeIngredientCells();
        const seen = new Set<string>();
        tiles = tiles.filter((t) => {
          if (blocked.has(idx(t.r, t.c))) return false;
          const key = `${t.r},${t.c}`;
        ...
        for (let r = 0; r < N; r++) {
          for (let c = 0; c < N; c++) {
            if (blocked.has(idx(r, c))) continue;
      ...
      const playSmallPop = async (cellIds: number[]) => {
        ...
        ordered.forEach(({ id, tv }, i) => {
          jobs.push((async () => {
            if (isTimedRunicClear) await wait(Math.min(260, i * 18));
            ...
            // v91: normal clears and natural Runic Clears use a timed, readable pop.
            // The old per-tile Runic explosion was too loud and made the clear feel like one big blast.
            candyFlashPop(cx, cy, 0xfff0b8, isTimedRunicClear ? 0.36 : 0.44);
            ...
            await Promise.all([
              addTween(tv.wrap.scale, { x: isTimedRunicClear ? 1.14 : 1.22, y: isTimedRunicClear ? 1.14 : 1.22 }, isTimedRunicClear ? 58 : 42, easeSoftBack),
              addTween(tv.wrap, { alpha: 1 }, isTimedRunicClear ? 58 : 42, easeOutQuart),
            ]);
            await Promise.all([
              addTween(tv.wrap.scale, { x: 0.54, y: 0.54 }, isTimedRunicClear ? 96 : 86, easeOutQuart),
              addTween(tv.wrap, { alpha: 0 }, isTimedRunicClear ? 96 : 86, easeOutQuart),
            ]);
            tv.wrap.alpha = 0;
            tv.busy = false;
          })());
      ...
      const showIngredientDropFx = (r: number, c: number, kind: IngredientKind = "coin") => {
        const p = xy(Math.min(N - 1, Math.max(0, r)), c);
        const cx = p.x + tileSize / 2;
        const cy = p.y + tileSize / 2;
        playSfx("special");
        addScorePopup(cx, cy, kind === "key" ? "+key" : "+coin", "special");
        spawnGuaranteedShockwave(cx, cy, 0xffd978, 0.86, "normal");
        lotusTargetGlowPop(cx, cy, 0xffd978, 0.72);
      };

      const applyIngredientFall = (clearSet: Set<number>) => {
        if (level.objectiveKind !== "ingredient" || !ingredients.length) return;
        let changed = false;
        for (const item of ingredients) {
          if (item.dropped) continue;
          let holesAtOrBelow = 0;
          for (let rr = item.r; rr < N; rr++) {
            if (clearSet.has(idx(rr, item.c))) holesAtOrBelow += 1;
          }
          if (holesAtOrBelow <= 0) continue;
          const oldR = item.r;
          item.r += holesAtOrBelow;
          item.renderFrom = { r: oldR, c: item.c };
          changed = true;
          if (item.r >= N - 1) {
            item.dropped = true;
            ingredientDropped = Math.min(level.ingredientTarget, ingredientDropped + 1);
            score += 150;
            showIngredientDropFx(N - 1, item.c, item.kind);
          }
        }
        if (changed) hud();
      };
      ...
      const dropAndFill = () => {
        ...
        // Floating ingredient cells are real occupied holes, so columns refill in segments
        // instead of letting runes pass through the key/coin.
        ...
        for (let c = 0; c < N; c++) {
          let r = N - 1;
          while (r >= 0) {
            if (blocked.has(idx(r, c))) {
              r--;
              continue;
            }

            const segmentBottom = r;
            while (r >= 0 && !blocked.has(idx(r, c))) r--;
            const segmentTop = r + 1;
            const survivors: Tile[] = [];

            for (let rr = segmentBottom; rr >= segmentTop; rr--) {
              const t = oldGrid[idx(rr, c)];
              if (t && !blocked.has(idx(t.r, t.c))) survivors.push(t);
            }
            ...
            for (let rr = segmentBottom; rr >= segmentTop; rr--) {
              ...
              if (existing) {
                existing.r = rr;
                existing.c = c;
                nextGrid[idx(rr, c)] = existing;
              } else {
                const color = randRefillRune(nextGrid, rr, c);
                nextGrid[idx(rr, c)] = newTile(rr, c, color);
              }
            }
          }
        }
      ...
      const clearCells = async (
        ...
        const spawnsToCreate: { k: number; special: Special; rune: Rune; conversion?: boolean }[] = [];
        ...
        const spawnedSpecials: { r: number; c: number; special: Special; conversion: boolean }[] = [];
        for (const nextSpawn of spawnsToCreate) {
          ...
          if (t) {
            ...
            if (nextSpawn.special !== "none") {
              ...
              spawnedSpecials.push({ r: loc.r, c: loc.c, special: nextSpawn.special, conversion: !!nextSpawn.conversion });
            }
          }
        }
        ...
        // v90: spawn sounds are special-specific and fired after the tile lands/appears.
        // Lotus spawn = spawn.mp3 only. Golden spawn = goldenspawn.mp3 only.
        // Only Runic Clear bonus conversions use clickclick.mp3.
        if (spawnedSpecials.length) {
          for (const spawned of spawnedSpecials) {
            playSpecialSpawnSfx(spawned.special, spawned.conversion);
        ...
        await cascadePulsePause(Math.max(1, combo), clearSet.size);
      ...
      const resolveCascades = async (firstSpawn: ReturnType<typeof chooseSpawn> | null) => {
        ...
        while (cascadeCount < maxCascadeLoops) {
          ...
          const stateKey = tiles
            .slice()
            .sort((a, b) => idx(a.r, a.c) - idx(b.r, b.c))
            .map((t) => `${t.r}${t.c}${t.color[0]}${t.special[0]}${t.id}`)
            .join("|");
          ...
          const didClear = await clearCells(clearSet, focus, 0, thisSpawn ?? undefined);
          spawn = null;
          if (!didClear) break;
      ...
      const playGoldenFlash = async (origin: Tile, dir: "h" | "v") => {
        ...
        // v100 Sacred Golden Slash:
        // Golden no longer visually deletes the whole line at once. The slash pulses first,
        // then sweeps outward from the activated Golden. Each tile cracks, sparkles, shakes,
        // and pops as the wave reaches it. Everything is Pixi Graphics-first for device safety.
        await pulseTileView(origin, 1.18, 54, 68);
        microShake(0.75, 76);
        candySparkBurst(cx, cy, color, 0.90);
        luxuryParticleBurst(["spark", "sparkle", "star"], cx, cy, 6, tileSize * 0.31, tileSize * 0.16, color, 180);

        const step = tileSize + gap;
        const beamLength = step * (N - 1) + tileSize * 0.86;
        const beam = new Container();
        beam.x = isH ? boardSize / 2 : cx;
        beam.y = isH ? cy : boardSize / 2;
        beam.alpha = 0;
        fxLayer.addChild(beam);

        const addBeamStroke = (width: number, tint: number, alpha: number) => {
          const g = new Graphics();
          if (isH) {
            g.moveTo(-beamLength / 2, 0);
            g.lineTo(beamLength / 2, 0);
          } else {
            g.moveTo(0, -beamLength / 2);
            g.lineTo(0, beamLength / 2);
          }
        ...
        addBeamStroke(Math.max(14, tileSize * 0.24), color, 0.16);
        addBeamStroke(Math.max(7, tileSize * 0.12), amber, 0.34);
        addBeamStroke(Math.max(2.5, tileSize * 0.045), hot, 0.84);
        beam.scale.set(isH ? 0.03 : 1, isH ? 1 : 0.03);
        Promise.all([
          gsapTo(beam, { alpha: 1 }, 26, "power2.out")
            .then(() => gsapTo(beam, { alpha: 0 }, 190, "power2.out")),
          gsapTo(beam.scale, { x: 1, y: 1 }, 132, "power4.out"),
        ]).then(() => beam.destroy({ children: true }));

        const originRing = new Graphics();
        originRing.circle(0, 0, tileSize * 0.30);
        originRing.stroke({ color: hot, alpha: 0.74, width: Math.max(1.8, tileSize * 0.032) });
        originRing.x = cx;
        originRing.y = cy;
        originRing.scale.set(0.46);
        fxLayer.addChild(originRing);
        Promise.all([
          gsapTo(originRing.scale, { x: 1.50, y: 1.50 }, 170, "power4.out"),
          gsapTo(originRing, { alpha: 0 }, 185, "power2.out"),
        ]).then(() => originRing.destroy());

        const cells: { r: number; c: number; d: number; side: number }[] = [];
        for (let d = 0; d < N; d++) {
          if (d === 0) {
            cells.push({ r: origin.r, c: origin.c, d, side: 0 });
            continue;
          }
          ...
          if (negR >= 0 && negR < N && negC >= 0 && negC < N) cells.push({ r: negR, c: negC, d, side: -1 });
          if (posR >= 0 && posR < N && posC >= 0 && posC < N) cells.push({ r: posR, c: posC, d, side: 1 });
        }

        const maxD = cells.reduce((m, cell) => Math.max(m, cell.d), 0);
        const impactDelay = 28;
        ...
        const spawnMovingHead = (side: -1 | 1) => {
          ...
          head.fill({ color: hot, alpha: 0.92 });
          head.circle(0, 0, tileSize * 0.230);
          head.fill({ color, alpha: 0.26 });
          head.x = cx;
          head.y = cy;
          head.blendMode = "add" as any;
          fxLayer.addChild(head);
          try { premiumGlow(head, color, 210, 1.24); } catch {}
          const endX = isH ? cx + side * endpointD * step : cx;
          const endY = isH ? cy : cy + side * endpointD * step;
          Promise.all([
            gsapTo(head, { x: endX, y: endY }, endpointD * impactDelay + 42, "power3.out"),
            gsapTo(head.scale, { x: 1.14, y: 1.14 }, endpointD * impactDelay + 42, "power2.out"),
          ]).then(() => Promise.all([
            gsapTo(head, { alpha: 0 }, 88, "power2.out"),
            gsapTo(head.scale, { x: 1.9, y: 1.9 }, 88, "power3.out"),
          ])).then(() => head.destroy());
        };
        ...
        const spawnSlashSegment = (tx: number, ty: number, side: number) => {
          ...
          const makeStroke = (width: number, tint: number, alpha: number, jitter = 0) => {
            const g = new Graphics();
            const length = tileSize * (side === 0 ? 0.80 : 1.18);
            if (isH) {
          ...
          makeStroke(Math.max(7, tileSize * 0.115), color, 0.22);
          makeStroke(Math.max(4, tileSize * 0.068), amber, 0.40, tileSize * 0.018);
          makeStroke(Math.max(2, tileSize * 0.034), color, 0.86);
          makeStroke(Math.max(1.2, tileSize * 0.017), hot, 0.95, -tileSize * 0.014);
          slash.rotation = isH ? (Math.random() - 0.5) * 0.035 : (Math.random() - 0.5) * 0.035;
          try { premiumGlow(slash, color, 190, 1.25); } catch {}
          Promise.all([
            gsapTo(slash, { alpha: 1 }, 28, "power2.out"),
            gsapTo(slash.scale, { x: isH ? 1.10 : 1.0, y: isH ? 1.0 : 1.10 }, 76, "power3.out"),
          ]).then(() => Promise.all([
            gsapTo(slash, { alpha: 0 }, 118, "power2.out"),
            gsapTo(slash.scale, { x: isH ? 1.26 : 1.0, y: isH ? 1.0 : 1.26 }, 118, "power2.out"),
          ])).then(() => slash.destroy({ children: true }));
        };
        ...
        const spawnCrackAndPop = (cell: { r: number; c: number; d: number; side: number }, orderIndex: number) => {
          ...
          spawnSlashSegment(tx, ty, cell.side);
          ...
          const crackLen = tileSize * 0.20;
          const crackCount = 3;
          for (let i = 0; i < crackCount; i++) {
            const a = (Math.PI * 2 * i) / crackCount + (isH ? 0 : Math.PI / 2) + (Math.random() - 0.5) * 0.50;
            crack.moveTo(0, 0);
            crack.lineTo(Math.cos(a) * crackLen * (0.72 + Math.random() * 0.28), Math.sin(a) * crackLen * (0.72 + Math.random() * 0.28));
          }
          crack.stroke({ color: orderIndex % 2 ? hot : color, alpha: 0.72, width: Math.max(1.0, tileSize * 0.018), cap: "round" } as any);
          fxLayer.addChild(crack);
          Promise.all([
            gsapTo(crack.scale, { x: 1.34, y: 1.34 }, 115, "power3.out"),
            gsapTo(crack, { alpha: 0 }, 125, "power2.out"),
          ]).then(() => crack.destroy());

          const ring = new Graphics();
          ring.circle(0, 0, tileSize * 0.18);
          ring.stroke({ color: hot, alpha: 0.62, width: Math.max(1.2, tileSize * 0.022) });
          ring.x = tx;
          ring.y = ty;
          ring.scale.set(0.44);
          fxLayer.addChild(ring);
          Promise.all([
            gsapTo(ring.scale, { x: 1.22, y: 1.22 }, 118, "power4.out"),
            gsapTo(ring, { alpha: 0 }, 130, "power2.out"),
          ]).then(() => ring.destroy());

          luxuryParticleBurst(["spark", "sparkle", "star"], tx, ty, 3, tileSize * 0.21, tileSize * 0.11, color, 150);
          if (orderIndex % 2 === 0) candySparkBurst(tx, ty, color, 0.36);
          ...
          if (tv) {
            tv.busy = true;
            const bumpX = isH ? (cell.side || 1) * tileSize * 0.035 : 0;
            const bumpY = isH ? 0 : (cell.side || 1) * tileSize * 0.035;
            Promise.all([
              gsapTo(tv.wrap, { x: tv.baseX + bumpX, y: tv.baseY + bumpY }, 24, "power2.out")
                .then(() => gsapTo(tv.wrap, { x: tv.baseX - bumpX * 0.55, y: tv.baseY - bumpY * 0.55 }, 28, "power2.out"))
                .then(() => gsapTo(tv.wrap, { x: tv.baseX, y: tv.baseY }, 30, "power2.out")),
              gsapTo(tv.wrap.scale, { x: 1.13, y: 1.13 }, 34, "back.out(1.45)")
                .then(() => gsapTo(tv.wrap.scale, { x: 0.56, y: 0.56 }, 92, "power3.in")),
              gsapTo(tv.wrap, { alpha: 0 }, 116, "power2.in"),
            ]).then(() => {
              tv.wrap.alpha = 0;
              tv.wrap.scale.set(0.56);
              tv.busy = false;
            });
          }
        };
        ...
        for (const [i, cell] of cells.entries()) {
          queueTimer(() => spawnCrackAndPop(cell, i), cell.d * impactDelay);
        }

        await wait(maxD * impactDelay + 205);
      };
      ...
      const showLotusRings = async (origin: Tile, color?: Rune, swappedWith?: Tile) => {
        ...
        rareLotusBloom(cx, cy, mainColor, 0.72, 0.74);
        lotusPetalExplosion(cx, cy, mainColor, 0.62, 0.72, true);
        ...
        ordered.forEach((t, i) => {
          ...
          queueTimer(() => {
            ...
            if (isSwapTarget || (!compact && i % 3 === 0)) spawnLayeredKennyRingBurst(tx, ty, mainColor, isSwapTarget ? 0.78 : 0.54, 0, compact);
            if (isSwapTarget || i % 5 === 0) lotusTargetBloom(tx, ty, mainColor, isSwapTarget ? 0.70 : 0.48);
...
const playLotus = async (origin: Tile, color: Rune, swappedWith?: Tile) => {
      ...
      const showGoldenLotusEclipse = async (lotus: Tile, golden: Tile, clear: Set<number>) => {
        ...
        const targets = tiles
          .filter((t) => clear.has(idx(t.r, t.c)) && t.id !== lotus.id && t.id !== golden.id)
          .sort((a, b) => {
      ...
      const chooseFinaleSpecial = (wave = 0): { special: Special; rune?: Rune } => {
        const roll = Math.random();
        // Timed Runic Clear: Golden is steady, Lotus is rarer and feels like the bloom.
        if (wave === 0 && roll < 0.34) return { special: "lotus", rune: "lotus" };
        if (roll < 0.18) return { special: "lotus", rune: "lotus" };
        if (roll < 0.72) return { special: "golden", rune: "golden" };
        return { special: "bomb" };
      };

      const scoreFinaleTarget = (t: Tile) => {
        const k = idx(t.r, t.c);
        if (fog[k] || t.special !== "none") return -9999;
        const center = (N - 1) / 2;
        let value = 120 - (Math.abs(t.r - center) + Math.abs(t.c - center)) * 10;
        const neighbors = [
          t.r > 0 ? tiles.find((z) => z.r === t.r - 1 && z.c === t.c) : null,
          t.r < N - 1 ? tiles.find((z) => z.r === t.r + 1 && z.c === t.c) : null,
          t.c > 0 ? tiles.find((z) => z.r === t.r && z.c === t.c - 1) : null,
          t.c < N - 1 ? tiles.find((z) => z.r === t.r && z.c === t.c + 1) : null,
        ].filter(Boolean) as Tile[];
        value += neighbors.filter((n) => n.color === t.color).length * 20;
        value += neighbors.filter((n) => n.special !== "none").length * 26;
        if (t.r >= 2 && t.r <= 4 && t.c >= 2 && t.c <= 4) value += 22;
        return value + Math.random() * 9;
      };

      const pickFinaleTargetTile = (used = new Set<number>()) => {
        const candidates = tiles.filter((t) => t.special === "none" && !fog[idx(t.r, t.c)] && !used.has(t.id));
        if (!candidates.length) return null;
        return candidates.slice().sort((a, b) => scoreFinaleTarget(b) - scoreFinaleTarget(a))[0] ?? null;
      };
      ...
      const activateFinaleSpecial = async (origin: Tile, wave: number) => {
        if (origin.special === "lotus") {
          await playLotus(origin, origin.color);
          return;
        }
        if (origin.special === "golden") {
          await playGolden(origin, wave % 2 === 0 ? "h" : "v");
          return;
        }
      ...
      const runRuneRushFinale = async () => {
        ...
        const stagedMoves = Math.min(8, unusedMoves);
        ...
        addBoardBannerText("RUNIC CLEAR!");
        void showBoardDimmer(0.13, unusedMoves > 0 ? 720 : 420);
        spawnGuaranteedShockwave(boardSize / 2, boardSize / 2, 0xffd978, 1.10, "goldenLotus");
        lotusLuxuryGlowBurst(boardSize / 2, boardSize / 2, 0xffd978, 1.08, 1.06);
        luxuryParticleBurst(["sparkle", "star", "magic", "spark"], boardSize / 2, boardSize / 2, 14, tileSize * 1.35, tileSize * 0.20, 0xffe6a3, 360);
        await wait(unusedMoves > 0 ? 390 : 260);
        ...
        for (let i = 0; i < stagedMoves; i++) {
          repairGrid();
          const target = pickFinaleTargetTile(usedTargets);
          if (!target) break;
          usedTargets.add(target.id);

          const next = chooseFinaleSpecial(i);
          target.special = next.special;
          if (next.rune) target.rune = next.rune;
          spawnedSpecialShieldIds.delete(target.id);

          message = `Runic Clear wave ${i + 1}/${stagedMoves}`;
          drawBoardBack();
          renderTiles();
          renderIngredients();
          renderFog();
          ...
          // True cascade after every staged leftover move:
          // clear -> gravity -> refill -> check new matches -> repeat until no matches.
        ...
        hud();
        spawnGuaranteedShockwave(boardSize / 2, boardSize / 2, 0xffd7f4, 0.92, "normal");
        await wait(360);
        return showRunicClearWin();
      };
      ...
      const checkWinFail = async () => {
        ...
        if (level.objectiveKind === "ingredient") won = ingredientDropped >= level.ingredientTarget;
        if (won) {
      ...
      const handleTap = (t: Tile) => {
        if (phase !== "idle" || fog[idx(t.r, t.c)] || hasActiveIngredientAt(t.r, t.c)) return;
        if (!selected) {
          selected = t;
          const tv = tileViews.get(t.id);
          if (tv) {
      ...
      const attemptSwap = async (a0: Tile, b0: Tile) => {
        ...
        if (fog[idx(a.r, a.c)] || fog[idx(b.r, b.c)] || hasActiveIngredientAt(a.r, a.c) || hasActiveIngredientAt(b.r, b.c)) return;
      ...
      const findHint = () => {
        for (const a of tiles) {
          if (fog[idx(a.r, a.c)] || hasActiveIngredientAt(a.r, a.c)) continue;
          for (const [dr, dc] of [[0, 1], [1, 0]] as [number, number][]) {
            const b = tiles.find((t) => t.r === a.r + dr && t.c === a.c + dc);
            if (!b || fog[idx(b.r, b.c)] || hasActiveIngredientAt(b.r, b.c)) continue;
      ...
      app.ticker.add((ticker: any) => {
        ...
        for (const child of ingredientLayer.children) {
          const cont = child as Container;
          for (const part of cont.children) {
            const floatMeta = (part as any).__ingredientFloat;
            if (floatMeta) {
              const lift = Math.sin(elapsed * 2.15 + floatMeta.seed);
              part.y = -tileSize * 0.030 + lift * floatMeta.amp;
              const s = 1 + Math.sin(elapsed * 1.6 + floatMeta.seed) * 0.018;
              (part as Container).scale.set(s);
            }

            for (const nested of (part as Container).children ?? []) {
              const glintMeta = (nested as any).__ingredientGlint;
              if (glintMeta) {
                const shimmer = (Math.sin(elapsed * 3.4 + glintMeta.seed) + 1) / 2;
                nested.alpha = 0.18 + shimmer * 0.72;
                (nested as Container).scale.set(0.82 + shimmer * 0.34);
              }
            }
          }
        }
