...
const BUILD_TAG = "pixifull-v93-golden-lightning-swipe";
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const playGoldenFlash = async (origin: Tile, dir: "h" | "v") => {
        ...
        // v93 Golden swipe: quick, snappy, layered lightning.
        // Pixi Graphics are the primary effect so it shows consistently on laptop, phone, and Base App.
        await pulseTileView(origin, 1.10, 34, 42);
        microShake(0.45, 52);
        ...
        const localX = (x: number) => x - startX;
        const localY = (y: number) => y - startY;
        beam.x = startX;
        beam.y = startY;
        ...
        const glowHalf = tileSize * 0.085;
        const auraHalf = tileSize * 0.048;
        const coreHalf = Math.max(1.6, tileSize * 0.020);
        const whiteHalf = Math.max(0.9, tileSize * 0.008);

        if (isH) {
          glow.roundRect(0, -glowHalf, len, glowHalf * 2, 999);
          aura.roundRect(0, -auraHalf, len, auraHalf * 2, 999);
          ...
          aura.roundRect(-auraHalf, 0, auraHalf * 2, len, 999);
        ...
        aura.fill({ color: 0xffb642, alpha: 0.24 });
        core.fill({ color: 0xffd46b, alpha: 0.88 });
        whiteHot.fill({ color: hot, alpha: 0.96 });
        beam.addChild(glow, aura, core, whiteHot);

        // Jagged lightning layers: thin white core + warm golden forks.
        const makeBolt = (boltIndex: number) => {
          const bolt = new Graphics();
          const steps = 10;
          const jitter = tileSize * (boltIndex === 0 ? 0.16 : 0.24);
          const side = boltIndex % 2 === 0 ? 1 : -1;
          if (isH) {
            bolt.moveTo(0, side * tileSize * 0.012);
            for (let i = 1; i <= steps; i++) {
              const y = (Math.random() - 0.5) * jitter + side * tileSize * 0.018;
              bolt.lineTo((len / steps) * i, y);
            }
          } else {
            bolt.moveTo(side * tileSize * 0.012, 0);
            for (let i = 1; i <= steps; i++) {
              const x = (Math.random() - 0.5) * jitter + side * tileSize * 0.018;
              bolt.lineTo(x, (len / steps) * i);
            }
          }
          bolt.stroke({
            color: boltIndex === 0 ? 0xffffff : boltIndex === 1 ? 0xfff0b7 : 0xffb949,
            alpha: boltIndex === 0 ? 0.80 : 0.46,
            width: boltIndex === 0 ? Math.max(1.25, tileSize * 0.022) : Math.max(0.9, tileSize * 0.013),
          });
          bolt.blendMode = "add" as any;
          beam.addChild(bolt);
          return bolt;
        };
        const bolts = [makeBolt(0), makeBolt(1), makeBolt(2), makeBolt(3)];

        premiumGlow(beam, color, 230, 1.55);
        premiumBloom(beam, 210, 0.72);
        premiumMotionBlur(beam, dir, 135);

        const originRing = new Graphics();
        originRing.circle(0, 0, tileSize * 0.28);
        originRing.stroke({ color: hot, alpha: 0.62, width: Math.max(1.5, tileSize * 0.026) });
        originRing.x = cx;
        originRing.y = cy;
        originRing.scale.set(0.35);
        originRing.blendMode = "add" as any;
        fxLayer.addChild(originRing);

        const head = new Graphics();
        head.circle(0, 0, tileSize * 0.095);
        head.fill({ color: hot, alpha: 0.88 });
        head.circle(0, 0, tileSize * 0.18);
        head.fill({ color, alpha: 0.18 });
        head.x = cx;
        head.y = cy;
        head.blendMode = "add" as any;
        fxLayer.addChild(head);
        premiumGlow(head, color, 180, 1.25);

        const addSlashTexture = (atX: number, atY: number, delay: number) => {
          queueTimer(() => {
            const slash = makeKennyPackParticle(atX, atY, tileSize * 0.48, 0.32, color, traceTextures.length ? traceTextures : undefined);
            if (!slash) return;
            slash.rotation = isH ? 0 : Math.PI / 2;
            slash.scale.set(0.18, 0.38);
            Promise.all([
              gsapTo(slash.scale, { x: 1.0, y: 1.18 }, 112, "power4.out"),
              gsapTo(slash, { alpha: 0, x: atX + (isH ? tileSize * 0.22 : 0), y: atY + (isH ? 0 : tileSize * 0.22) }, 132, "power2.out"),
            ]).then(() => slash.destroy());
          }, delay);
        };

        // Leading swipe head + texture streaks. Very fast so the move feels crisp.
        addSlashTexture(cx, cy, 0);
        addSlashTexture((cx + endX) / 2, (cy + endY) / 2, 44);
        addSlashTexture(endX, endY, 82);

        candySparkBurst(cx, cy, color, 0.62);
        gsapTo(originRing.scale, { x: 1.35, y: 1.35 }, 120, "power4.out");
        gsapTo(originRing, { alpha: 0 }, 128, "power2.out").then(() => originRing.destroy());

        await gsapTo(beam, { alpha: 1 }, 16, "power2.out");
        await Promise.all([
          gsapTo(beam.scale, { x: 1, y: 1 }, 58, "power4.out"),
          gsapTo(head, { x: endX, y: endY }, 76, "power4.out"),
        ]);
        ...
        touched.forEach((cell, i) => {
          queueTimer(() => {
            ...
            // Small snappy impact pops along the line, not a slow explosion.
            const ring = new Graphics();
            ring.circle(0, 0, tileSize * 0.18);
            ring.stroke({ color: i % 2 ? hot : color, alpha: 0.52, width: Math.max(1, tileSize * 0.018) });
            ring.x = tx;
            ring.y = ty;
            ring.scale.set(0.48);
            ring.blendMode = "add" as any;
            fxLayer.addChild(ring);
            Promise.all([
              gsapTo(ring.scale, { x: 1.18, y: 1.18 }, 108, "power4.out"),
              gsapTo(ring, { alpha: 0 }, 112, "power2.out"),
            ]).then(() => ring.destroy());

            luxuryParticleBurst(["spark", "sparkle", "star"], tx, ty, 3, tileSize * 0.22, tileSize * 0.12, color, 130);
            if (i % 2 === 0 && sparkleTextures.length) {
              const sp = makeKennyPackParticle(tx, ty, tileSize * 0.20, 0.34, i % 4 === 0 ? hot : color, sparkleTextures);
              if (sp) {
                sp.scale.set(0.16);
                Promise.all([
                  gsapTo(sp.scale, { x: 0.78, y: 0.78 }, 120, "power3.out"),
                  gsapTo(sp, { alpha: 0, rotation: sp.rotation + 0.35 }, 128, "power2.out"),
                ]).then(() => sp.destroy());
              }
            }
            ...
            const tv = tile ? tileViews.get(tile.id) : null;
            if (tv) {
              gsapTo(tv.wrap.scale, { x: 1.10, y: 1.10 }, 30, "back.out(1.35)")
        ...
        await wait(46);
        await Promise.all([
          gsapTo(beam, { alpha: 0 }, 56, "power3.out"),
          gsapTo(beam.scale, { x: isH ? 1.02 : 1, y: isH ? 1 : 1.02 }, 56, "power3.out"),
          gsapTo(head, { alpha: 0 }, 56, "power2.out"),
          gsapTo(head.scale, { x: 1.9, y: 1.9 }, 56, "power3.out"),
        ]);
