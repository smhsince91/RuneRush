...
const BUILD_TAG = "pixifull-v79-matte-gradient-ultra-specials";
...
export default function RuneRushPixiBoard({ levelIndex, onHud, soundOn = true, onLevelComplete, onLevelFailed }: Props) {
  ...
  const playOriginalSfx = (key: SfxKey) => {
    ...
    // Important: do not claim success if the browser has not actually loaded the file.
    // When this returns false, the WebAudio synth fallback plays immediately so sounds do not vanish.
    try {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const createTileView = (t: Tile, start?: { r: number; c: number }, spawned = false) => {
        ...
        // Matte gradient built with simple Pixi Graphics so it is consistent across devices.
        // Bright lifted center, slightly darker sides, deepest bottom edge.
        const matteCenter = new Graphics();
        matteCenter.roundRect(-socketSize * 0.34, -socketSize * 0.34, socketSize * 0.68, socketSize * 0.68, Math.max(9, radius - 7));
        matteCenter.fill({ color: 0xf1f6f2, alpha: 0.062 });
        matteCenter.mask = stoneMask;
        wrap.addChild(matteCenter);

        const matteMid = new Graphics();
        matteMid.roundRect(-socketSize * 0.43, -socketSize * 0.43, socketSize * 0.86, socketSize * 0.86, Math.max(10, radius - 5));
        matteMid.stroke({ color: 0xffffff, alpha: 0.038, width: Math.max(1, Math.round(tileSize * 0.018)) });
        matteMid.mask = stoneMask;
        wrap.addChild(matteMid);

        const sideShade = new Graphics();
        sideShade.roundRect(-tileHalf + 2, -tileHalf + 2, socketSize - 4, socketSize - 4, Math.max(10, radius - 2));
        sideShade.stroke({ color: 0x000000, alpha: 0.18, width: Math.max(3, Math.round(tileSize * 0.052)) });
        sideShade.mask = stoneMask;
        wrap.addChild(sideShade);

        const centerStone = new Graphics();
        centerStone.roundRect(-socketSize * 0.35, -socketSize * 0.35, socketSize * 0.70, socketSize * 0.70, Math.max(9, radius - 7));
        centerStone.fill({ color: 0xf7fbf8, alpha: 0.035 });
        ...
        innerMatte.stroke({ color: 0xffffff, alpha: 0.045, width: Math.max(1, Math.round(tileSize * 0.012)) });
        ...
        carvedEdge.stroke({ color: 0x020303, alpha: 0.42, width: Math.max(2, Math.round(tileSize * 0.040)) });
        wrap.addChild(carvedEdge);

        const outerLip = new Graphics();
        outerLip.roundRect(-tileHalf + 2, -tileHalf + 2, socketSize - 4, socketSize - 4, Math.max(9, radius - 2));
        outerLip.stroke({ color: 0xffffff, alpha: 0.028, width: Math.max(1, Math.round(tileSize * 0.016)) });
        outerLip.mask = stoneMask;
        wrap.addChild(outerLip);

        const bottomDepth = new Graphics();
        bottomDepth.roundRect(-tileHalf + 4, socketSize * 0.08, socketSize - 8, socketSize * 0.29, Math.max(8, radius - 5));
        bottomDepth.fill({ color: 0x000000, alpha: 0.10 });
        ...
        topCarve.fill({ color: 0xffffff, alpha: 0.018 });
        topCarve.mask = stoneMask;
        wrap.addChild(topCarve);

        const edgeVignette = new Graphics();
        edgeVignette.roundRect(-tileHalf + 3, -tileHalf + 3, socketSize - 6, socketSize - 6, Math.max(9, radius - 4));
        edgeVignette.stroke({ color: 0x000000, alpha: 0.12, width: Math.max(2, Math.round(tileSize * 0.03)) });
        edgeVignette.mask = stoneMask;
        wrap.addChild(edgeVignette);
        ...
        if (tex) {
          ...
          const runeScale = t.special === "none" ? 0.68 : 0.78;
      ...
      const showLotusRings = async (origin: Tile, color?: Rune, swappedWith?: Tile) => {
        ...
        const clearPreview = new Set<number>();
        addLotusCells(clearPreview, origin, chosenColor);
        const previewTargets = tiles.filter((t) => clearPreview.has(idx(t.r, t.c)) && t.id !== origin.id);
        ...
        // v76 Lotus:
        // Keep the current Lotus movement/zap, but add a luxury ring-burst preview on every rune that will clear.
        // Performance guard: every target still gets a visible ring, but large target groups use compact particles.
        const targetCount = previewTargets.length;
        const manyTargets = targetCount > (isMobileView ? 13 : 18);
        const hugeTargets = targetCount > (isMobileView ? 22 : 28);
        const delayStep = hugeTargets ? 6 : manyTargets ? 7 : 8;
        const maxDelay = hugeTargets ? 185 : manyTargets ? 220 : 255;

        microShake(0.9, 76);
        void showBoardDimmer(0.07, 205);
        await pulseTileView(origin, 1.2, 40, 46);
        spawnGuaranteedLotusTargetMarker(cx, cy, mainColor, 1.12, true, 0);
        spawnLayeredKennyRingBurst(cx, cy, mainColor, 1.28, 0, false);
        lotusLuxuryGlowBurst(cx, cy, mainColor, 1.06, 1.16);
        candyFlashPop(cx, cy, mainColor, 1.02);
        await wait(72);

        const ordered = previewTargets
          .slice()
          .sort((a, b) => {
        ...
        ordered.forEach((t, i) => {
          const delay = Math.min(maxDelay, i * delayStep);
          queueTimer(() => {
            if (cancelled) return;
            const p = xy(t.r, t.c);
            ...
            const compact = !isSwapTarget && (manyTargets || fxQuality < 0.58);
            const burstPower = isSwapTarget ? 1.36 : compact ? 0.88 : 1.08;

            spawnGuaranteedLotusTargetMarker(tx, ty, mainColor, burstPower, isSwapTarget, 0);
            spawnLayeredKennyRingBurst(tx, ty, mainColor, burstPower, 0, compact);
            lotusSparkleTargetRing(tx, ty, mainColor, 0);
            lotusCandyZap(cx, cy, tx, ty, mainColor, 0);
            lotusTargetGlowPop(tx, ty, mainColor, isSwapTarget ? 1.1 : compact ? 0.68 : 0.86);

            if (isSwapTarget || !compact) {
              candySparkBurst(tx, ty, mainColor, isSwapTarget ? 0.74 : 0.52);
            } else if (i % 2 === 0) {
              candySparkBurst(tx, ty, mainColor, 0.26);
            }

            const tv = tileViews.get(t.id);
            if (tv) {
              const up = isSwapTarget ? 1.28 : compact ? 1.12 : 1.2;
              gsapTo(tv.wrap.scale, { x: up, y: up }, 42, "back.out(1.6)")
                .then(() => gsapTo(tv.wrap.scale, { x: compact ? 0.94 : 0.9, y: compact ? 0.94 : 0.9 }, 52, "power3.out"))
                .then(() => gsapTo(tv.wrap.scale, { x: 1, y: 1 }, 48, "power3.out"));
            }
          }, delay);
        });

        await wait(Math.min(650, ordered.length * delayStep + 220));
