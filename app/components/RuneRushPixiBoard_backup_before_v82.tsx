...
const BUILD_TAG = "pixifull-v81-lotus-restore-sound-sync";
...
export default function RuneRushPixiBoard({ levelIndex, onHud, soundOn = true, onLevelComplete, onLevelFailed }: Props) {
  ...
  useEffect(() => {
    ...
    for (const key of Object.keys(ORIGINAL_SFX) as SfxKey[]) {
      ...
      for (const src of ORIGINAL_SFX[key].slice(0, 5)) {
        ...
        audio.preload = "auto";
        audio.volume = key === "bad" ? 0.34 : key === "golden" || key === "lotus" ? 0.50 : key === "convert" ? 0.48 : 0.42;
  ...
  const playOriginalSfx = (key: SfxKey) => {
    ...
    // Try the first actually-loaded file. If none are ready, return false so the WebAudio
    // fallback plays on time instead of a delayed/broken HTMLAudioElement sound.
    for (const audio of candidates) {
      try {
        if (audio.error || audio.networkState === HTMLMediaElement.NETWORK_NO_SOURCE || audio.readyState < HTMLMediaElement.HAVE_CURRENT_DATA) continue;
        audio.pause();
        audio.currentTime = 0;
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const clearCells = async (
        ...
        // v81: fire the sound/ring after the new special is drawn, so sound matches the visual pop.
        for (const spawned of spawnedSpecials) {
          playSfx(spawned.naturalRunicClear ? "convert" : "spawn");
