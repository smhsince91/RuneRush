...
export default function RuneRushPixiFullPage() {
  ...
  const boardKey = useMemo(() => `pixi-full-v104-${levelIndex}-${resetKey}`, [levelIndex, resetKey]);
  ...
  const updateAudioSettings = (patch: Partial<AudioSettings>) => {
    setAudioSettings((prev) => {
      ...
      if (typeof patch.musicVolume === "number") {
        ...
        if (audio) audio.volume = next.musicVolume;
  ...
  const playMusicIfAllowed = () => {
    ...
    syncYoutubeMusic(musicVolume, true);
  };

  useEffect(() => {
    console.log("[Rune Rush Page] loaded v104-music-immediate-volume-fix");
  }, []);
      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; }
        ...
        .soundMenu {
          ...
          bottom: calc(100% + 9px);
          width: min(304px, calc(var(--game-w) - 16px));
          border-radius: 20px;
          padding: 10px;
          background:
